...
async def _extract_items_from_photos(sender_id: str, photos: List[Dict[str, str]]) -> List[dict]:
    """
    Sequentially extract material items from a batch of photos.
    Ensures the engagement messages (got it / almost there) are emitted once.
    """
    ...
    for idx, photo in enumerate(photos):
        ...
        if img_path and os.path.exists(img_path):
            try:
                ...
                print("⚠  Image file not found while batching:", img_path)

        work_coro = extract_materials(caption, img_b64)
        if idx == 0:
            ...
            items = await extract_materials(caption, img_b64)
...
async def _process_photo_batch(
    ...
    if summary is None:
        # _finalize_bulk_batch already updates state with an error message on failure
        state.setdefault(
            "latest_respons",
            "Sorry, there was an error while saving your order. Please try again.",
        )
        state.setdefault("uoc_next_message_type", "plain")
        state.setdefault("needs_clarification", True)
        return state

    message, message_type, extra = summary
    state.update(
        latest_respons=message,
        uoc_next_message_type=message_type,
        uoc_next_message_extra_data=extra,
        needs_clarification=True,
        agent_first_run=False,
    )
    return state
...
async def new_user_flow(state: AgentState, crud: ProcurementCRUD  ) -> AgentState:
    ...
    img_b64 = None
    img_path = state.get("image_path")
    batched_photos = state.pop("batched_images", None)
    photos_to_process: List[Dict[str, str]] = []

    if batched_photos:
        photos_to_process = batched_photos
        first_photo = batched_photos[0]
        state["image_path"] = first_photo.get("image_path")
        state["caption"] = first_photo.get("caption", "")
        img_path = state["image_path"]
    elif img_path:
        photos_to_process = [{"image_path": img_path, "caption": state.get("caption", "")}]
...
"What would you like to do now?"
            ...
            if photos_to_process:
                print(
                    "Procurement Agent:::: new_user_flow : processing photo batch:",
                    len(photos_to_process),
                )
                items = await _extract_items_from_photos(sender_id, photos_to_process)
            else:
        ...
        state.setdefault("procurement_details", {})["materials"] = items or []
